...
impl Pipeline {
    ...
    async fn process(&mut self, update: Update, datasource_id: DatasourceId) -> CarbonResult<()> {
        ...
        match update {
            Update::Account(account_update) => {
                // ensure stats map has this datasource
                self.ensure_stats_entry(&datasource_id);
            ...
            Update::BlockDetails(block_details) => {
                // BlockDetails don't carry datasource stats beyond slot
    ...
    #[inline]
    fn extract_account_key(acc: &crate::datasource::AccountUpdate) -> AccountKey {
        let mut hasher = AHasher::default();
        hasher.write(&acc.account.data);
        let data_fingerprint = hasher.finish();
        AccountKey {
